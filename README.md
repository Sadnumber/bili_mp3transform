...
- **图形界面版按文件体积区分音视频**：同一视频的两条流中，视频流体积总是明显大于音频流。
  B 站的清晰度编号并不固定（视频流除 `300xx` 外还有 AV1 的 `100xx` 等，音频流有 `30216` / `30232` / `30280` / `30250` 杜比等），
  仅靠编号匹配会误判，因此编号只在两者体积相差不足 10% 时作为辅助判断。
- 最后还会用 ffmpeg 探测每个流的**实际轨道类型**来校正结果——体积和编号都只是猜测，
